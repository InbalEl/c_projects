/*
* Circular buffer is a data structure with limited capacity,
* allowing re-use of the same memory areas.
*
* Attributes:
...
* -	Read - Read "count" bytes from a given buffer, and writes it to
* -	a given dest. O(n)
...
* - June 17th 09:17 Ver:  1.4 Fixing: CBufferWrite
							  CBufferread complexity: O(n)
							  Include guard updated to fixed format
							  Errno under CBufferWrite
							  CBufferread has been updated
...
* Args:
*   - c_buffer - pointer to buffer given by user
*   - src - source of data to write into bu
ffer
